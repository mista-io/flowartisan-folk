...
export class AliasEditor extends LitElement {
    ...
    renderFeature(feature, remainingPath) {
        ...
        const clickable = (feature.has_children || feature.level === 0) &&
    ...
    handleDialogClick(evt) {
        ...
        if (button.name === 'Save') {
            ...
            postJSON(this.getEndpoint() + 'boundaries/' + this.editFeature.osm_id + '/', payload).then(() => {
                this.fetchFeature();
            });
        }
    ...
    getOptions(response) {
        return response.json.filter((option) => option.level > 0);
    }
    getOptionsComplete(newestOptions) {
        return newestOptions.length === 0;
    }
    renderOptionDetail(option) {
        const labelStyles = {
            marginTop: '3px',
            marginRight: '3px',
        };
        const aliasList = option.aliases.split('\n');
        const aliases = aliasList.map((alias) => alias.trim().length > 0
            ? html `
    ...
    render() {
        ...
        const selectedFeature = this.path[this.path.length - 1];
        const mapFeature = selectedFeature.children.length === 0
...
], AliasEditor.prototype, "editFeatureAliases", void 0);
//# sourceMappingURL=AliasEditor.js.map