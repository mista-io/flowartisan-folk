...
export class ContactHistory extends RapidElement {
    ...
    updated(changedProperties) {
        ...
        if (changedProperties.has('refreshing') &&
            ...
            fetchContactHistory(false, this.endpoint, this.ticket, null, after)
                ...
                fetchedEvents = fetchedEvents.filter(item => {
                    const found = !!this.eventGroups.find(g => !!g.events.find(exists => exists.created_on === item.created_on &&
        ...
        if (changedProperties.has('tickets')) {
            ...
            const closedTickets = (this.tickets || []).filter(ticket => ticket && ticket.status === 'closed');
            for (const closed of closedTickets) {
                const child = stickyBin.querySelector(`[data-sticky-id="${closed.uuid}"]`);
                if (child) {
    ...
    handleScroll() {
        ...
        stickies.forEach((sticky) => {
            ...
            if (!sticky.classList.contains('pinned')) {
                const eventElement = sticky.firstElementChild;
                ...
                const eventElement = sticky.eventElement;
    ...
    renderEvent(event) {
        switch (event.type) {
            ...
            case Events.TICKET_OPENED: {
                const ticketEvent = event;
                const activeTicket = !this.ticket || ticketEvent.ticket.uuid === this.ticket;
    ...
    render() {
        ...
        const unfetchedTickets = this.eventGroups.length > 0 && this.tickets
