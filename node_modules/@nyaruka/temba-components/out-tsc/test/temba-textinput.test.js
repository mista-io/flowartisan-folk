...
export const getInputHTML = (attrs = { value: 'hello world' }) => {
    return `<temba-textinput ${getAttributes(attrs)}></temba-textinput>`;
};
export const createInput = async (def) => {
    const parentNode = document.createElement('div');
    parentNode.setAttribute('style', 'width: 250px;');
    const input = await fixture(def, { parentNode });
    return input;
};
describe('temba-textinput', () => {
    ...
    it('shows placeholder', async () => {
        const input = await createInput(getInputHTML({ placeholder: 'Enter some text' }));
    ...
    it('should render textarea', async () => {
        const input = await createInput(getInputHTML({ value: 'hello world', textarea: true }));
        assert.instanceOf(input, TextInput);
    ...
    it('takes internal input changes', async () => {
        const input = await createInput(getInputHTML({ value: 'hello world' }));
    ...
    it('does not take internal input changes for disabled', async () => {
        const input = await createInput(getInputHTML({ value: 'hello world', disabled: true }));
        // trigger a change on our internal widget
        const widget = input.shadowRoot.querySelector('.textinput');
        expect(widget.tagName).to.equal('INPUT');
    ...
    it('does not take internal textarea changes for disabled', async () => {
        const input = await fixture(getInputHTML({ value: 'hello world', textarea: true, disabled: true }));
        // trigger a change on our internal widget
        const widget = input.shadowRoot.querySelector('.textinput');
        expect(widget.tagName).to.equal('TEXTAREA');
        expect(widget.disabled).to.equal(true);
    ...
    it("doesn't advance cursor on GSM character replacement", async () => {
        const input = await createInput(getInputHTML({ value: 'hello world', textarea: true, gsm: true }));
        input.value = 'Let’s try some text with a funny tick.';
        // focus our widget, move back a few spots and insert some text
        await click('temba-textinput');
        await pressKey('ArrowLeft', 5);
        await type('replaced ');
        expect(input.value).to.equal("Let's try some text with a funny replaced tick.");
    });
    ...
    it('updates on date selection', async () => {
        ...
        await waitFor(500);
        // click on a new date
        const newDate = document.querySelectorAll(".flatpickr-day[aria-label='April 21, 2020']")[1];
        newDate.click();
...
//# sourceMappingURL=temba-textinput.test.js.map