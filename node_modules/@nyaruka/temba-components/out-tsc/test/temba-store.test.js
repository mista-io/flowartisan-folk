...
import './utils.test';
const createStore = async (def) => {
    const store = (await fixture(def));
...
describe('temba-store', () => {
    ...
    it('completion schema', async () => {
        const store = await createStore("<temba-store completion='/test-assets/store/editor.json'></temba-store>");
        assert.equal(store.getCompletionSchema().types.length, 16);
        assert.equal(store.getFunctions().length, 80);
    });
    it('globals', async () => {
        const store = await createStore("<temba-store globals='/test-assets/store/globals.json'></temba-store>");
...
//# sourceMappingURL=temba-store.test.js.map